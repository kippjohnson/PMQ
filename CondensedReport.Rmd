...
library(ggplot2)
library(grid) # multiplot for ggplot2
...
```{r Openness questions, echo=FALSE, warning=FALSE, fig.height=6, fig.width=10}
...
infile3$Q10c <- infile3$Q10
infile3$Q10c[infile3$Q10c==2] <- 1
infile3$Q10c[infile3$Q10c==3] <- 1
infile3$Q10c[infile3$Q10c==4] <- 2
infile3$Q10c[infile3$Q10c==5] <- 2
...
g1 <- ggplot(data=tmp_infile, aes(x=factor(variable), fill=(factor(value)))) + 
      ...
      #ggtitle("Medical student openness toward incorporating\ngenomic tools into clinical practice") + 
      xlab("") + ylab('Frequency') +
...
```{r Openness questions figure, echo=FALSE, warning=FALSE, fig.height=6, fig.width=10}
infile3$Q2c <- infile3$Q2
infile3$Q2c[infile3$Q2c==2] <- 1
infile3$Q2c[infile3$Q2c==3] <- 1
infile3$Q2c[infile3$Q2c==4] <- 2
infile3$Q2c[infile3$Q2c==5] <- 2

infile3$Q4c <- infile3$Q4
infile3$Q4c[infile3$Q4c==2] <- 1
infile3$Q4c[infile3$Q4c==3] <- 1
infile3$Q4c[infile3$Q4c==4] <- 2
infile3$Q4c[infile3$Q4c==5] <- 2

infile3$Q5c <- infile3$Q5
infile3$Q5c[infile3$Q5c==2] <- 1
infile3$Q5c[infile3$Q5c==3] <- 1
infile3$Q5c[infile3$Q5c==4] <- 2
infile3$Q5c[infile3$Q5c==5] <- 2

infile3$Q8c <- infile3$Q8
infile3$Q8c[infile3$Q8c==0] <- 1
infile3$Q8c[infile3$Q8c==2] <- 1
infile3$Q8c[infile3$Q8c==3] <- 1
infile3$Q8c[infile3$Q8c==4] <- 2
infile3$Q8c[infile3$Q8c==5] <- 2

tmp_infile <- infile3[,c('Q2c','Q4c','Q5c','Q8c')];
tmp_infile <- melt(tmp_infile);

levels(tmp_infile$variable) <- c("Clinical experience is more\nimportant than using a patient’s genetic\ninformation to make decisions ",
                                 "Not willing to prescribe\ndifferent medications or doses of medications\nbased on a patient’s genetic information ",
                                 "Clinicians know better than academic\n researchers how to treat patients ",
                                 "Research-based genome-guided\nprescribing tools are not clinically useful " )

#levels(tmp_infile$variable) <- factor(tmp_infile$variable, levels=rev(tmp_infile$variable))

g2 <- ggplot(data=tmp_infile, aes(x=factor(variable), fill=(factor(value)))) + 
      geom_histogram(aes(), position="stack") +
      #ggtitle("Medical student openness toward incorporating\ngenomic tools into clinical practice") + 
      xlab("") + ylab('Frequency') +
      scale_fill_discrete(name='', labels=c('\nNot At All/\nTo a Slight Extent/\nTo a Moderate Extent\n','\nTo a Great Extent/\nTo a Very Great Extent\n')) +
...
```{r Combined figures for 9 vs. 28.2, 29.1, 30.1, echo=FALSE}
## 28.2
infile3a <- infile3[!is.na(infile3$Q9),]

infile3a$Q28.2.fNA <- (as.factor(infile3a$Q28.2c))
levels(infile3a$Q28.2.fNA) <- c('MS1','MS2',"MS3+")

g2 <- ggplot(data=infile3a, aes(factor(Q28.2.fNA), fill=Q9cf), aes(y=..count../sum(..count..))) + 
      geom_bar(position="fill", binwidth=0.25 )+ 
      xlab("Medical School Year") + ylab('Percent') + 
      scale_fill_discrete(name='Response') + coord_flip() + 
      guides(fill=guide_legend(reverse=TRUE)) + scale_y_continuous(labels=percent)
      #ggtitle("I think that it is important to learn about personalized medicine")

g2;

## 29.1
infile3a$Q29.1.fNA <- (as.factor(infile3a$Q29.1))
infile3a_naomit <- infile3a[which(!is.na(infile3a$Q29.1.fNA)) , ]
levels(infile3a_naomit$Q29.1.fNA) <- c('No','Yes')

g3 <- ggplot(data=infile3a_naomit, aes(factor(Q29.1.fNA), fill=Q9cf), aes(y=..count../sum(..count..))) + 
      geom_bar(position="fill", binwidth=0.25 )+ 
      xlab("Enrolled in Dual Degree Program") + ylab('Percent') + 
      scale_fill_discrete(name='Response') + coord_flip() + 
      guides(fill=guide_legend(reverse=TRUE)) + scale_y_continuous(labels=percent)
...
## 30.1
infile3a$Q30.1.fNA <- (as.factor(infile3a$Q30.1))
levels(infile3a$Q30.1.fNA) <- c('No','Yes',"Unsure")
new.levels<-c("No/\nUnsure","Yes","No/\nUnsure")
infile3a$Q30.1.fNA <- new.levels[infile3a$Q30.1.fNA]

g4 <- ggplot(data=infile3a, aes(factor(Q30.1.fNA), fill=Q9cf), aes(y=..count../sum(..count..))) + 
      geom_bar(position="fill", binwidth=0.25 )+ 
      #stat_bin(geom='text', aes(label='99%'), hjust=2 ) +
      xlab("Interest in a Research Career") + ylab('Percent') + 
      scale_fill_discrete(name='Response') + coord_flip() + 
      guides(fill=guide_legend(reverse=TRUE)) + scale_y_continuous(labels=percent)

g4;

g5 <- ggplot(data=infile3a, aes(x=factor(Q9cf), y=EBPAS))
g5 <- g5 + geom_boxplot(aes(fill=factor(Q9cf))) 
g5 <- g5 + scale_x_discrete(labels=c('Not at all/\nTo a slight extent/\nTo a moderate extent','To a great extent\nTo a very great extent'))
g5a <- g5 + xlab("") + ylab('Attitude Score')
g5 <- g5 + xlab("") + ylab('Attitude Score') + coord_flip() 
g5n <- g5 + theme(legend.position="none")
...
grid.text("I think that it is important to learn about personalized medicine", vp = viewport(layout.pos.row = 1, layout.pos.col = 1))
...
grid.newpage()
pushViewport(viewport(layout = grid.layout(5, 1, heights = unit(c(1, 4, 4,4,4), "null"))))
grid.text("I think that it is important to learn about personalized medicine", vp = viewport(layout.pos.row = 1, layout.pos.col = 1))
print(g2, vp = viewport(layout.pos.row = 2, layout.pos.col = 1))
print(g3, vp = viewport(layout.pos.row = 3, layout.pos.col = 1))
print(g4, vp = viewport(layout.pos.row = 4, layout.pos.col = 1))
print(g5n, vp = viewport(layout.pos.row = 5, layout.pos.col = 1))
...
grid.newpage()
pushViewport(viewport(layout = grid.layout(4, 2, heights = unit(c(1,4,4,1), "null"))))
grid.text("I think that it is important to learn about personalized medicine", vp = viewport(layout.pos.row = 1, layout.pos.col = 1:2))
print(g2n, vp = viewport(layout.pos.row = 2, layout.pos.col = 1))
print(g3n, vp = viewport(layout.pos.row = 2, layout.pos.col = 2))
print(g4n, vp = viewport(layout.pos.row = 3, layout.pos.col = 1))
print(g5n, vp = viewport(layout.pos.row = 3, layout.pos.col = 2))
grid.rect(gp=gpar(fill="#F8766D"), vp= viewport(layout.pos.row=4, layout.pos.col=1), width=0.25, hjust=1,vjust=0.5)
grid.text('Not at all/\nTo a slight extent/\nTo a moderate extent',
          vp= viewport(layout.pos.row=4, layout.pos.col=1), gp=gpar(cex=0.80), hjust=-0.25,vjust=0.5)

grid.rect(gp=gpar(fill="#00BFC4"), vp= viewport(layout.pos.row=4, layout.pos.col=2), width=0.25,hjust=1,vjust=0.5)
grid.text('To a great extent/\nTo a very great extent', 
          vp= viewport(layout.pos.row=4, layout.pos.col=2),gp=gpar(cex=0.80), hjust=-0.25,vjust=0.5)




grid.newpage()
pushViewport(viewport(layout = grid.layout(5, 2, heights = unit(c(1,4,4,0.5,0.5), "null"))))
grid.text("I think that it is important to learn about personalized medicine", vp = viewport(layout.pos.row = 1, layout.pos.col = 1:2))
print(g2n, vp = viewport(layout.pos.row = 2, layout.pos.col = 1))
print(g3n, vp = viewport(layout.pos.row = 2, layout.pos.col = 2))
print(g4n, vp = viewport(layout.pos.row = 3, layout.pos.col = 1))
...
grid.rect(gp=gpar(fill="#F8766D"), vp= viewport(layout.pos.row=4, layout.pos.col=1:2), width=0.2, hjust=2,vjust=0.5)
grid.text('Not at all/To a slight extent/To a moderate extent',
          vp= viewport(layout.pos.row=4, layout.pos.col=1:2), gp=gpar(cex=0.80), hjust=0.55,vjust=0.5)

grid.rect(gp=gpar(fill="#00BFC4"), vp= viewport(layout.pos.row=5, layout.pos.col=1:2), width=0.2,hjust=2,vjust=0.5)
grid.text('To a great extent/To a very great extent          ', 
          vp= viewport(layout.pos.row=5, layout.pos.col=1:2),gp=gpar(cex=0.80), hjust=0.55,vjust=0.5)


infile3a$`Question Response` <- as.factor(infile3a$Q9cf)

g5 <- ggplot(data=infile3a, aes(x=`Question Response`, y=EBPAS))
g5 <- g5 + geom_point(data=infile3a, aes(x=`Question Response`, y=EBPAS, color=`Question Response`),shape=15,size=0)
g5 <- g5 + guides(color=guide_legend(title='Question Response')) 
g5 <- g5 + theme(legend.key=element_blank())
g5 <- g5 + guides(col=guide_legend(override.aes=list(shape=15,size=15)),title='Question Response')
g5 <- g5 + geom_boxplot(aes(fill=factor(Q9cf)), show_guide=FALSE) 
g5 <- g5 + scale_x_discrete(labels=c('Not at all/\nTo a slight extent/\nTo a moderate extent','To a great extent\nTo a very great extent'))
g5a <- g5 + xlab("") + ylab('Attitude Score')
g5a <- g5a + theme(legend.position="top")
g5a <- g5a + scale_x_discrete(labels=NULL)
g5a

grid.newpage()
pushViewport(viewport(layout = grid.layout(4, 2, heights = unit(c(1,4,4,4), "null"), widths=unit(c(4,2.5),"null") )))
grid.text("I think that it is important to learn about personalized medicine", vp = viewport(layout.pos.row = 1, layout.pos.col = 1:2), gp=gpar(cex=1.5))
print(g2n, vp = viewport(layout.pos.row = 2, layout.pos.col = 1))
print(g3n, vp = viewport(layout.pos.row = 3, layout.pos.col = 1))
print(g4n, vp = viewport(layout.pos.row = 4, layout.pos.col = 1))
print(g5a, vp = viewport(layout.pos.row = 2:4, layout.pos.col = 2))




#main=textGrob("Total Data and Image", gp=gpar(cex=1), just="top")
#grid.arrange(main,g2,g3,g4,ncol=1)#, main='\nI think that it is important to learn about personalized medicine')

#multiplot(g2,g3,g4)
#ggsave(filename="~/Dropbox/Medical Student PM Manuscript/Figures/Figure 1.png",width=12,height=9, g)
...
```{r Random paper statistics}
# respondent ages
prop.table(table(infile3$Q26))

# Question 1 collapsed categories
infile3$Q1c <- infile3$Q1
infile3$Q1c[infile3$Q1c==2] <- 1
infile3$Q1c[infile3$Q1c==3] <- 1
infile3$Q1c[infile3$Q1c==4] <- 2
infile3$Q1c[infile3$Q1c==5] <- 2
prop.table(table(infile3$Q1c))

# Question 7 collapsed categories
infile3$Q7c <- infile3$Q7
infile3$Q7c[infile3$Q7c==2] <- 1
infile3$Q7c[infile3$Q7c==3] <- 1
infile3$Q7c[infile3$Q7c==4] <- 2
infile3$Q7c[infile3$Q7c==5] <- 2
prop.table(table(infile3$Q7c))

# Question 12 collapsed categories
infile3$Q12c <- infile3$Q12
infile3$Q12c[infile3$Q12c==2] <- 1
infile3$Q12c[infile3$Q12c==3] <- 1
infile3$Q12c[infile3$Q12c==4] <- 2
infile3$Q12c[infile3$Q12c==5] <- 2
prop.table(table(infile3$Q12c))

# Question 9 collapsed categories
infile3$Q9c <- infile3$Q9
infile3$Q9c[infile3$Q9c==2] <- 1
infile3$Q9c[infile3$Q9c==3] <- 1
infile3$Q9c[infile3$Q9c==4] <- 2
infile3$Q9c[infile3$Q9c==5] <- 2
prop.table(table(infile3$Q9c))

```

```{r summary statistics for knowledge/ability}
mean(infile3$genomicknowledge)
mean(infile3$genomiccomfort)

sd(infile3$genomicknowledge)
sd(infile3$genomiccomfort)

range(infile3$genomicknowledge)
range(infile3$genomiccomfort)

```

```{r EBPAS-GII Plot}
g1 <- ggplot(data=infile3[!is.na(factor(infile3$Q28.2c)), ], aes(x=factor(Q28.2c), y=EBPAS) )
g1 <- g1 + geom_boxplot() 
g1 <- g1 + scale_x_discrete(labels=c('MS1','MS2','MS3+'))
g1 <- g1 + ggtitle("EBPAS-GII Score") + xlab("") + ylab('Cumulative Score')
g1 <- g1 + coord_flip();
g1;
